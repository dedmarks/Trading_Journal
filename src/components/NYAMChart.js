...
function NYAMChart() {
    ...
    useEffect(() => {
      ...
      tradee.forEach((trade) => {
        if (trade.data.status === 'Win' && trade.data.session === 'NY AM') {
          NYAMwins++
        }else if(trade.data.status === 'Lose' && trade.data.session === 'NY AM'){
          NYAMloses++
        }
      });
  ...
  return (
    <div className="c">
        <h3 className="session__title">NY AM Session</h3>
        <div className="chart__container2">
          <Doughnut data={data}/>
        </div>
        <span className="session__percentage">{((NYAMwins/(NYAMwins+NYAMloses))*100).toFixed(2)}%</span>
    </div>
  )
}

export default NYAMChart